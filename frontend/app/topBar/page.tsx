...
//                       activeSection === item.id && pathname === "/"
//                         ? "scale-x-0" // Hidden when active
//                         : "scale-x-0 group-hover:scale-x-100" // Visible on hover only
//                     }`}
//                   ></span>
//                 </button>
//               ))}
...
//                     className="relative p-2.5 text-gray-400 hover:text-gray-200 transition-colors duration-300 cursor-pointer rounded-lg hover:bg-gray-700/50"
//                     title="Notifications"
//                   >
//                     <FaBell size={18} />
//                     {notifications.length > 0 && (
//                       <span className="absolute top-1 right-1 inline-flex items-center justify-center w-5 h-5 text-xs font-bold text-white bg-red-500 rounded-full border-2 border-gray-800">
...
//           {/* Login Placeholder (No Profile Option) */}
//           {!user ? (
//             <button
//               onClick={() => setIsLoginOpen(true)}
...
export default function TopBar({
  ...
  const formatTime = (timeStr: string) => {
    return timeStr; // Simplified for demo
  };
